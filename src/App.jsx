...
const [usage, setUsage] = useState(0);
  const loadBiz = async (tk, uid) => { setLd(true); const d = await sb.get("businesses",tk,"user_id=eq."+uid); if(Array.isArray(d)&&d.length>0){setBiz(d[0]); var monthAgo=new Date(Date.now()-30*24*60*60*1000).toISOString(); var r=await fetch(SB_URL+"/rest/v1/chat_logs?business_id=eq."+d[0].id+"&created_at=gte."+monthAgo+"&select=id",{headers:{"apikey":SB_KEY,"Authorization":"Bearer "+tk,"Prefer":"count=exact"}}); var c=parseInt(r.headers.get("content-range")?.split("/")[1]||"0"); setUsage(c);}else{const n=await sb.add("businesses",{user_id:uid,name:"Uusi korjaamo"},tk);if(Array.isArray(n)&&n.length>0)setBiz(n[0]);} setLd(false); }; const loadBiz = async (tk, uid) => { setLd(true); const d = await sb.get("businesses",tk,"user_id=eq."+uid); if(Array.isArray(d)&&d.length>0){setBiz(d[0]); try{var r=await fetch(SB_URL+"/rest/v1/rpc/get_usage",{method:"POST",headers:{"Content-Type":"application/json","apikey":SB_KEY,"Authorization":"Bearer "+tk},body:JSON.stringify({bid:d[0].id})}); var c=await r.json(); setUsage(typeof c==="number"?c:0);}catch(e){setUsage(0);}}else{const n=await sb.add("businesses",{user_id:uid,name:"Uusi korjaamo"},tk);if(Array.isArray(n)&&n.length>0)setBiz(n[0]);} setLd(false); };
